...
if [[ $vm_setting == 0 ]]; then
  ...
  gui_install="xorg-server xorg-drivers xorg-xinit virtualbox-guest-utils xf86-video-vesa"
fi
...
pacstrap -i /mnt base base-devel grub zsh vim git netctl dialog wpa_supplicant mc

# configure the system
genfstab -pU /mnt >> /mnt/etc/fstab
(
  echo "echo \"$set_hostname\" > /etc/hostname";
  echo "ln -sf /usr/share/zoneinfo/Europe/Minsk /etc/localtime";
 ...
 #echo "echo \"LANG=ru_RU.UTF-8\" > /etc/locale.conf";
  ...
  echo "pacman -S intel-ucode --noconfirm";
...
dhcpcd
pacman -Syy
pacman -S $gui_install mc --noconfirm
pacman -S $de_install --noconfirm
#pacman -S networkmanager network-manager-applet ppp --noconfirm
#pacman -S xf86-video-intel lib32-intel-dri --noconfirm
#pacman -S transmission-gtk transmission-cli p7zip unrar gpm bash-completion gparted wine --noconfirm
